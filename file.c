...
*/
	 ...
	 */
	err = utCalendar_cal( *dimval, file->udunits, 
		&year0, &month0, &day0, &hour0, &min0, &sec0, d->calendar );
	if( err == 0 ) {
		err = utInvCalendar_cal( year0, month0, day0, hour0, min0, sec0, 
...
/*
err = utCalendar_cal( *dimval, var->first_file->udunits, 
 ...
 * of the dimension value, which is either NC_FLOAT or NC_CHAR.  Make sure
